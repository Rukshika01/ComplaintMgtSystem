<?php
 ...
 <html>
   ...
   <div class="div">
     ...
     <?php
     $no=1;
       $db=mysql_query("SELECT * FROM `officer` WHERE id = $id_d");
       while($data=mysql_fetch_array($db)) {
           $name = $data['name'];
             }
     ?>
     <h2>Forwarding these details <?php echo $name ; ?></h2>
      <br><br>
 ...
 <?php
   ...
   while( $arry=mysql_fetch_array($query1) ) {
     ...
     $TypesOfComplaint = $arry['TypesOfComplaint'];
 ...
 $TypesOfComplaint_e = mysql_real_escape_string($TypesOfComplaint);
   ...
   if(($status)==4){
     $query = mysql_query("SELECT ref_no FROM `view_cmp` WHERE ref_no=$ref");
       if (mysql_num_rows($query) != 0){
           #$message =  "This message is already send to the selected Officer";
         ...
         <div class ='text-right' id='close'>
          ...
          <p>Message successfully sent to the selected Officer</p>
         </div>";

       }else{
         mysql_query("INSERT INTO `view_cmp` VALUES ('0','$ref_e','$name_e','$email_e','$phone_no_e','$TypesOfComplaint_e','$complain_e','$id_d')") or die(mysql_error());

         $message =   "<div class='alert succ' id='msg'>
         <div class ='text-right' id='close'>
         <svg class='pointer' fill='#FFF' height='24' viewBox='0 0 24 24' width='24'>
             <path d='M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z'/>
             <path d='M0 0h24v24H0z' fill='none'/>
         </svg>
           <p class='text-center'>Message successfully sent to the selected Officer</p>
         </div> </div>";
       }
   }else{
          $message = "<div class='alert errr' id='msg'>
 ...
 <form class="" action="" method="post">
   ...
   <input type="text" name="status" placeholder="2 + 2 = ?">
   <input type="submit" class="frd" value="send"> 
   <p>Solve to prove that you're a human!!</p>
 </form>
 <br><br><br>
